...
class WinStruct(object, metaclass=CMeta):
    """
    ...
    def _link_cstructs(self, obj):
        ...
        for fname, subobj in obj.__filtermap__.items():
            ...
            elif isinstance(subobj, WinStruct):
        ...
        if struct:
            ...
            for fn, val in fields:
                if fn == name:
                    ...
                    return
        super(WinStruct, self).__setattr__(name, value)
    ...
    def __getattribute__(self, name):
        ...
        """
        try:
            struct = super(WinStruct, self).__getattribute__('__struct__')
            if struct:
                fields = struct._fields_
                for fn, val in fields:
                    if fn == name:
                        val = struct.__getattribute__(name)
                        tests = (WinStruct.FilteredStruct, WinUnion.FilteredStruct, ct.Array)
                        if any([isinstance(val, t) for t in tests]):
                            fm = super(WinStruct, self).__getattribute__('__filtermap__')
            ...
            pass
        return super(WinStruct, self).__getattribute__(name)


class WinUnion(WinStruct, metaclass=CMeta):
